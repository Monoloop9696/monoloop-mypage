...
src/
  ...
  auth/AuthContext.jsx   ログイン状態・admin判定（/api/set-role でクレーム同期）＋**24時間無操作で自動ログアウト**（localStorage の ml_last_active を1分間隔で監視。期限切れ時は ml_idle_logout を立て、ログイン画面で案内表示）
  lib/
...
api/                     ※Vercel Hobby は関数12個まで。現在11個（下記7参照）
  ...
  send-code / register / set-role / student-account(退会/復元＋action=lastLogin で最終ログイン取得) / public-cohorts / reset-password /
...
- **面談の担当者**の選択肢も `templates` に `_type:"interviewer"`（{name, order}）で保存。面談タブの「管理する」から名称変更（既存の面談記録も自動更新）・削除・並び替え（手動▲▼／あいうえお順・面談数順）が可能
- **面談記録**は `templates` コレクションに `_type:"meeting"`（{uid, name, grad, date, time, interviewer, kind, note, next}）で保存。**templates は管理者のみ read/write なので学生には見えない**（専用コレクションだとルール再デプロイが必要になるため）
...
- **セレクトの値を「保存値から推測」しない**: Journeyのリンク種別は `link` の文字列から種別を判定していたため、URL入力中に空/スキーム無しになった瞬間に種別が変わり入力欄が消えて保存できなかった（2026-09-01修正）。種別は `linkType` として明示保存する。
...
**管理（AdminApp）**:
- **表示モード切替（PC表示 / スマホ表示）**: ヘッダーのボタンで切替、localStorage `ml_admin_pc` に保存（初回は画面幅1024px以上でPC）。PC表示＝最大1600px幅・左サイドバーナビ（下部タブは非表示）・概況/記事/LINE配信は2カラム。
- 年度スイッチャー(3件横並び＋pastプルダウン)。**イベント/アンケートの作成・編集画面は右からのドロワー**（ヘッダーと保存ボタンを固定、中身だけスクロール）。概況のイベント/アンケート一覧は**新しい3件まで表示＋残りは折りたたみ**。内定者タブの**アカウント配布情報は既定で折りたたみ**。
- 概況: 集計／**イベントCRUD**(下書き・回答期限(日付)・**対象エリア(現住所/実家/どちらも)＋個別対象者モーダル**・最終受付終了(手動)・**会場到着ボタンの有無/文言(作成フォーム＋公開後もカードから変更可)**・**出欠/到着の管理側編集＋キャンセル理由**・**公開済みイベントも「編集」で全項目を変更可（出欠/到着データは保持）**・**公開後も「対象者を調整」で個別に表示/非表示（後から登録した学生もこの一覧に出る）**・履歴から作成)／**アンケートCRUD**(動的設問・テンプレ/履歴から作成・下書き・対象者(イベント参加者)・**説明欄(desc)**・**セクション＋回答による分岐(単一選択の選択肢ごとに 次へ/指定セクション/終了)**・**対象エリア(現住所/実家/どちらも)＋対象者を確認・個別調整モーダル（公開後も「対象者を調整」で変更可）**・回答集計/CSV・**選択式は選択肢タップで回答者一覧モーダル**)／Journey(**ドラッグ並び替え**・リンク設定＝イベント/アンケートは**対象を1件指定可**)／お知らせ。
...
- **面談タブ**: 面談の記録（日付/時刻/種別/担当者=プルダウン・その場で追加可/内容/次回予定）をドロワーで追加・編集・削除。表示は「日付順」と「内定者別（未実施が一目で分かる）」の2モード＋横断検索。内定者の詳細にも直近3件を表示。
- **モチベーション調査（パルス）**: 分析タブから毎回同じ設問(PULSE_QUESTIONS・5段階評価4問＋自由記述)で実施。保存は surveys に `pulse:true`（学生が読めるのは surveys のため）、回答は通常どおり responses。設問タイプ **scale** を新設（学生側は1〜5ボタン＋両端ラベル、集計は平均と分布）。
- **要フォロー自動判定**（AI不使用のルールベース）: スコアの低さ/前回比の低下/調査未回答/ログイン間隔/イベント欠席・未回答/アンケート未回答/面談間隔/LINE未連携を点数化し「高/中/低」と理由＋フォロー案を提示。重み付けは followUpOf() にまとまっている。
- **分析タブ**: 採用ファネル(承諾率/辞退率)・内定→承諾日数(平均/中央値)・エリア別・流入経路別(承諾率)・内定出し月別・承諾月別・大学別・エンゲージメント(LINE連携率/イベント出席率/アンケート回答率)・面談実施率。全年度/卒年度で切替。グラフはCSSバーで描画（ライブラリ追加なし）。
