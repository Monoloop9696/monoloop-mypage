...
- 概況: 集計／**イベントCRUD**(下書き・回答期限(日付)・**対象エリア(現住所/実家/どちらも)＋個別対象者モーダル**・最終受付終了(手動)・**出欠/到着の管理側編集＋キャンセル理由**・履歴から作成)／**アンケートCRUD**(動的設問・テンプレ/履歴から作成・下書き・対象者(イベント参加者)・回答集計/CSV)／Journey(**ドラッグ並び替え**・リンク設定)／お知らせ。
